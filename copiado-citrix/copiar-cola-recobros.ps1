...
if ($archivoDestino -eq "") {
    ...
    $archivoDestino = "salida\santander-cola-recobros-" + $mes + ".csv"
}
...
foreach($linea in $lineas) {
    ...
    switch($estado) {
		0 {
			...
			if ($primerElemento -ne "1 Cola Recobros") {
				$linea
        ...
        1 {
           ...
           if ($primerElemento -eq "RESUMEN POR SERVICIO") {
            $estado = 2
           } 
        }
        2 {
          # Leer línea de cabecera
          if ($primerElemento -eq "Servicio") {
            $estado = 3
          } else {
